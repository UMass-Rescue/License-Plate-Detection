...
class PlateFinder:
    ...
    def preprocess(self, input_img):
        imgBlurred = cv2.GaussianBlur(input_img, (15, 15), 0)  # old window was (5,5)
        gray = cv2.cvtColor(imgBlurred, cv2.COLOR_BGR2GRAY)  # convert to gray
        bfilter = cv2.bilateralFilter(gray, 11, 17, 17)
        # edged = cv2.Canny(bfilter, 30, 200)
        sobelx = cv2.Sobel(bfilter, cv2.CV_8U, 1, 0, ksize=3)  # sobelX to get the vertical edges
...
def get_license_plates(video_path):
    res = []
        ...
        if ret == True:
            ...
            if possible_plates is not None:
                for i, p in enumerate(possible_plates):
                    ...
                    res.append((str(recognized_plate), str(cap.get(cv2.CAP_PROP_POS_MSEC))))
                    # print("Plate Number: "+str(recognized_plate)+",", "Time Stamp: "+str(cap.get(cv2.CAP_PROP_POS_MSEC)))
                    # else:
                    #     print("Plate Number: "+str(recognized_plate)+",", "Time Stamp: "+str(cap.get(cv2.CAP_PROP_POS_MSEC)))
                    # # cv2.imshow('plate', p)
                    # # if cv2.waitKey(25) & 0xFF == ord('q'):
...
if __name__ == "__main__":
    res = get_license_plates('test_videos/video.MOV')
    print("FINAL", res)
    # findPlate = PlateFinder()
